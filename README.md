...
- **SOS-объявление** о пропаже за минуту: радиус, награда, фото, район, рассылка соседям и в Telegram.
...
- **Telegram-бродкаст** районного канала.
...
- Магазин адресников (каталог, корзина в cookie, заказ), пристройство, B2B-партнёры, подписка «Лапка+».
...
| Тесты | **Vitest** + E2E-харнесс `tools/verify-all.mjs` | юниты + 20 живых сюит |
...
- **E2E-сюиты** — `node tools/verify-all.mjs` (нужен поднятый dev-сервер + БД): **20 сюит**
  одной командой, реальные транзакции против `:3000` с проверкой БД — от полного прогона
  интерактивных флоу (`e2e-flows`) до точечных: кросс-матчинг, пульс, OG, экспорт, фильтры,
  календарь, плакаты, sitemap, RSS, жизненный цикл, редактирование, soft-404, матч-алерты,
  идемпотентность кредита, заявка «это моя собака», листовка района.
...
- [x] **Распространение** — OG, Telegram, sitemap (SEO), RSS (пропажи + находки).
- [x] **Комьюнити/витрина** — лента, волонтёры, «Пульс», магазин, пристройство, партнёры, Лапка+.
- [x] **Качество** — 20 E2E-сюит, два ревью-прохода, a11y-аудит, фикс soft-404.
